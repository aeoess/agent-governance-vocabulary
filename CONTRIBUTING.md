...
- **Automated checks:** every PR runs the crosswalk validator (`npm run validate`, which includes the `fixtures/` scope checks) plus a DCO check and contributor reputation triage. CodeQL and OSSF Scorecard watch the repository itself. A red check always states its exact fix in the output. The DCO sign-off must match the commit author and is a self-attestation of the right to submit, not identity verification. Required checks gate pull requests; maintainer pushes to `main` run the same validation after landing rather than before.
